...
- **What was built (tasks 1-7, spec `docs/superpowers/specs/2026-07-10-verification-gate-design.md`,
  ...
  exposed three failure classes in the old critique loop: (F1) critique only checked
  "is every claim supported", never "does the answer address the question", so a
  grounded non-answer about self-attention passed as clean for a question about the
  Transformer's optimizer; a fabricated "1/135th of the compute" for ELECTRA also passed,
  because nothing compared specific numbers in the answer against the evidence text.
  (F2) three-way comparison answers went deep on one document and shallow on the rest,
  because nothing tied the final answer back to the decomposed sub-questions. (F3) the
  router over-escalated single-document two-part questions ("optimizer AND warmup
  schedule" for ONE paper) from medium to hard, costing ~10x latency and, in one case,
  actively hurting answer quality by diluting the evidence pool.
  Three cooperating fixes:
  1. **Deterministic routing guard** (`implicated_documents` in `src/ingest.py`, applied
     in `router.py`): an LLM `hard` label downgrades to `medium` when the question's
  ...
  3. **Verification gate** (`src/nodes/verify.py`, replacing the old critique node): a
     ...
     critique never checked) and support (claims vs. evidence, absorbed from the old
     critique). Failure-type-aware healing replaces the single retry path: `citations`,
     `fabrication`, `responsiveness`, and `coverage` regenerate against the same pool
     with explicit feedback; `support` re-searches on the restated claims (sanitized,
     never overwriting the original `sub_questions`, so coverage keeps checking the real
     decomposition). Budget-exhausted answers get an honest "could not be fully
...
- **Live verification (task 8) - measured, not assumed.** Ollama running with
  ...
  4. **PDF validation (step 4).** Cross-checked Q1, Q3, Q9, Q10 factual claims against
     source PDF text via PyMuPDF grep. All confirmed present except the Q13 issues
  ...
  5. **Threshold tuning (step 5): not changed, decision recorded.** Swept nothing;
     ...
     needlessly; no query's *final* `failure_type` was ever `coverage`, i.e. it never
     caused a budget-exhausted honest-exit (the over-triggering failure mode). Left at
     **0.45** in `src/config.py`, with the comment there now noting this run as the
     empirical check.
