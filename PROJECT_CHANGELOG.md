...
3. **Verification gate** (`src/nodes/verify.py`) — a deterministic Stage 1 (citation
   ...
   `verification_warnings`, which the UI renders as a caution caption instead of
   presenting the answer as clean.

**Live-verified (see §9 for the full measured results).** Offline suite 105/105;
routing accuracy unchanged at 93.3%; Q1's optimizer/warmup answer confirmed correct
against source and correctly labeled when uncertain; the two mis-escalated medium
...
Ollama running `qwen2.5:7b`. The 13-query suite from the design spec's evidence section
was re-run end-to-end through `build_graph()` (the same graph the UI uses) and checked
against source PDF text via PyMuPDF grep.
...
the named escape hatch (a larger local verifier model) applies here too if it recurs.
