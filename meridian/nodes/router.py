...
def _fresh_run(route: str, trace: list[str]) -> RAGState:
    """The router opens every question, so it is also where per-question state
    is cleared -- `iterations` back to 0 and the heal fields emptied, so a
    reused session cannot leak a previous question's verdict."""
    return {
        "route": route,
        "mode_label": config.MODE_LABELS[route],
        "iterations": 0,
        "failure_type": "",
        "verify_feedback": "",
        "retry_queries": [],
...
def route_question(state: RAGState) -> RAGState:
    ...
    if is_meta_question(question):
        ...
        return _fresh_run(config.ROUTE_META, ["router → meta"])
